...
export class NpmAuth extends Step {
  ...
  async installStep() {
    ...
    const hasAccount = await yesOrNo("Do you already have an account in npm?");
    if (!hasAccount) {
      console.log("Let's create one..");
      console.log(
        `We need to create a user using your ${red(`@monday.com`)} user.`
      );
    ...
    await runCommand(
      `nvm use 18.19 && npm login --username "${username}" --password "${password}"`,
      {},
      {
        printWhile: true,
        async onData(data) {
          const fullUrl =
            /https:\/\/www.npmjs.com\/login\?next=\/login\/cli\/[a-z0-9-]+/.exec(
              data
            );
          if (fullUrl) {

...
export class InstallXcode extends Step {
  ...
  async installStep() {
    ...
    const hasAccount = await yesOrNo(
      "Do you have an AppStore Connect Apple ID with a @monday.com email?"
    );

    if (!hasAccount) {
      console.log(
        "Ask a team member to invite you to monday's AppStore Connect account and run this script again."
      );

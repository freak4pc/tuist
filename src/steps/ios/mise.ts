...
export class InstallMise extends Step {
  ...
  async installStep() {
    ...
    if (fs.existsSync(this.projectPath())) {
      await runCommand(
        `cd ${this.projectPath()} && ${this.misePath} install`,
        {},
        { stdio: Stdio.Inherit }
      );
    } else {
      console.log(
        yellow(
          `⚠️: mise didn't auto-install dependencies since the iOS folder doesn't exist at ${this.projectPath()}. Run 'mise install' in the iOS folder to install dependencies.`
        )
      );
    }
  }

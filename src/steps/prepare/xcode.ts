...
export class InstallXcodeCLI extends Step {
    ...
    try {
      if (!(await runCommand("xcode-select -p")).trim()) {
        return {
          valid: false,
          reason:
            "Xcode command line tools are not installed. Command not found",
        };
      }
    } catch (e: any) {
      if (e.message.includes("command not found")) {
        return {
          valid: false,
          reason:
            "code command line tools are not installed. Command not found",
        };
      }
      return {
        valid: false,
        reason: `Xcode error while checking. ${e.message}`,
      };
    }

    return {
      valid: true,
      reason: "Already installed Xcode Command Line Tools",
    };
  }
  ...
  async installStep() {
    ...
    await runCommand(
      `PROD=$(softwareupdate -l | grep "\*.*Command Line" | tail -n 1 | sed 's/^[^C]* //') && softwareupdate -i "$PROD" --verbose;`,
      {},
      { stdio: Stdio.Inherit }
    );
  }

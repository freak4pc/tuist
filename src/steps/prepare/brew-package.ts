...
export class InstallBrewPackages extends Step {
  ...
  constructor(
    packages: string[],
    { force = false, cask = false }: { force?: boolean; cask?: boolean } = {}
  ) {
    super();
  ...
  async installCheck() {
    for (const curpackage of this.packages) {
      try {
        const shortName =
          curpackage.split("/")[curpackage.split("/").length - 1];
        const showList = await runCommand(
          `brew list ${curpackage}`,
          {},
          { detailedError: false }
        );
        ...
        if (!showList.includes(shortName)) {
          ...
          return {
            valid: false,
            reason: `Package ${curpackage} is not listed`,
          };
        }
      } catch (e: any) {
        if (e.message.includes("No such keg")) {
          return {
            valid: false,
            reason: `Package ${curpackage} is not installed`,
          };
        }
        return {
          valid: false,
          reason: `Package ${curpackage} has error while checked. ${e.message}`,
        };
      }
    }

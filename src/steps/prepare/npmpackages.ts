...
export class InstallNpmPackages extends Step {
    ...
    async checkPreinstall(): Promise<{valid: boolean, reason?: string, fixAction?: () => Promise<void> }> {
        const mndyPackages = this.packages.filter(pkg => pkg.includes('@mondaydotcom'));
        if (mndyPackages.length > 0) {
            for(const pkg of mndyPackages) {
                try {
                    await runCommand(`npm view ${pkg}`, {}, { detailedError: false });
                }
                catch(e: any) {
                    if(e.message.includes('code E404') || e.message.includes('code E403')) {
                        return { valid: false, reason: `You don't have access to install ${pkg}. ${e.message}`, fixAction: async () => {
                            console.log("Press any key to open the form to request access to install the package.")
                            await pressAnyKeyToContinue();
                            open("https://forms.monday.com/forms/913463924b1367c3ec3c952c5985a5b9?r=use1")
                        }};
                    }
                }
            }
    ...
    async installCheck() {
        for(const pkg of this.packages) {
            try {
                const hasPkg = await runCommand(`npm list -g ${pkg}`, {}, { detailedError: false });

...
export async function hasEnvSetTo(env: string, expectedValue: string): Promise<boolean> {
    const currentEnvValue = (await runCommand(`source ~/.zshrc > /dev/null 2>&1 || true && echo $${env}`)).trim();
